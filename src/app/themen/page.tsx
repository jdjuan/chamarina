    ...
    <main>
      ...
      <section className="px-5 py-20 sm:px-8 lg:px-10">
        <div className="mx-auto grid max-w-7xl gap-8">
          {focusAreas.map((area) => (
            <article
              className="grid gap-8 rounded-[2.25rem] border border-[#B9CFDD] bg-white p-7 shadow-[0_18px_48px_rgba(20,54,59,0.06)] lg:grid-cols-[220px_1fr]"
              key={area.title}
            >
              <div>
              ...
              <div>
                ...
                {area.items ? (
                  <div className="mt-5 flex flex-wrap gap-2">
                    {area.items.map((item) => (
                      <span
                        className="inline-flex items-center gap-2 rounded-2xl border border-[#B9CFDD] bg-white px-3.5 py-2 text-sm font-medium text-[#0D2744] shadow-[0_10px_24px_rgba(20,54,59,0.06)]"
                        key={item}
                      >
                        <span
                          aria-hidden="true"
      ...
      <section className="bg-[#F4F8FB] px-5 py-16 sm:px-8 lg:px-10">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="max-w-2xl text-3xl font-semibold text-[#0D2744]">
